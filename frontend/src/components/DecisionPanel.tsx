...
export function DecisionPanel({ actionError, actionMessage, onBackToPayment, onCancelPayment, onConfirmPayment, onVerifyRecipient, pendingAction = null, result }: DecisionPanelProps) {
  ...
  return (
    <Card className="guardian-panel decision-card decision-card--held" id="guardian-decision">
      ...
      <div className="decision-card__actions">
        {(requiresGuidance || !result.lifecycle.independent_guidance_acknowledged) && !isBlocked ? <Button disabled={isActionPending} onClick={onVerifyRecipient} variant="primary">{result.intervention.primary_button}</Button> : null}
        {confirmAllowed ? <Button disabled={isActionPending} loading={isConfirming} onClick={onConfirmPayment} variant="secondary">{isConfirming ? 'Confirming with Guardian' : 'Continue to Payment'}</Button> : null}
        <Button disabled={isActionPending} loading={isCancelling} onClick={onCancelPayment} variant="ghost">{isCancelling ? 'Cancelling payment' : 'Cancel Payment'}</Button>
      </div>
